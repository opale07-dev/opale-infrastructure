...
variable "data_allowed_cidr" {
  type        = string
  description = "Trusted CIDR allowed to reach the Data Index internal API port."

  validation {
    condition     = trimspace(var.data_allowed_cidr) != "" && can(cidrhost(var.data_allowed_cidr, 0))
    error_message = "data_allowed_cidr must be a non-empty valid CIDR (example: 198.51.100.20/32)."
  }
}
